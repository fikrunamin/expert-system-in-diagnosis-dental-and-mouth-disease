...
$sql = "SELECT reports.report_id, users.fullname, users.phone_number, users.email_address, users.job, users.age, reports.disease, reports.disease, reports.symptoms, reports.treatments, reports.created_at FROM users JOIN reports ON users.user_id=reports.user_id WHERE reports.user_id=:user_id ORDER BY `created_at` DESC";
$stmt = $db->prepare($sql);
